...
r2 <- readRDS("Data_r2_range_models.rds")
```

look at temperature since LGM
```{r}
...
m_metric <- m_metric_range_vel_clim_hf
...
for(i in r){
  l[,2][l$r == i] <- round(m_metric_range_clim[[i]][["hf"]][[1]][["DIC"]],2)
  l[,3][l$r == i] <- round(m_metric_range_hf[[i]][["hf"]][[1]][["DIC"]],2)
  l[,4][l$r == i] <- round(m_metric_range_clim_hf[[i]][["hf"]][[1]][["DIC"]],2)
  l[,5][l$r == i] <- round(m_metric_range_vel[[i]][["hf"]][[1]][["DIC"]],2)
  l[,6][l$r == i] <- round(m_metric_range_vel_clim_hf[[i]][["hf"]][[1]][["DIC"]],2)
  l[,7][l$r == i] <- round(m_metric_null[[i]][["hf"]][[1]][["DIC"]],2)
}
names(l) <- c("Metrics", "Climate", "Human_footprint", "Climate*Human_footprint", "Vel", "Climate*HF*Vel", "Null")
l[,2] <- as.numeric(l[,2])
l[,3] <- as.numeric(l[,3])
l[,4] <- as.numeric(l[,4])
l[,5] <- as.numeric(l[,5])
...
dic_table$Delta_DIC[5] <- dic_table[5,5] - dic_table[5,5]
...
dic_table[5,5] <- paste("**", dic_table[5,5], "**", sep = "")
...
# sum <- list()
# for(i in r) {
# sum[[i]] <- as.data.frame(summary(m_metric_range_vel_clim_hf[[i]][["hf"]][[1]])$solutions);
# sum[[i]] <- setDT(sum[[i]], keep.rownames = TRUE)[]
# sum[[i]] <- sum[[i]][2:6,]
#sum[[i]]$rn <- c( "HF", "MAP", "MAT", "MAP_var","MAT_var")
# sum[[i]]$rn <- factor(sum[[i]]$rn,
#                      levels = c( "MAP_var","MAT_var","MAP", "MAT",  "HF"))
#}



## woaw....
check <- broom.mixed::tidy(m_metric_range_vel_clim_hf[["total.area"]][["hf"]][[1]], effects = "fixed", conf.int = TRUE, conf.level = 0.95)
...
pairs(m[, which(names(m) %in% c("hf_mean", "mat_mean", "mat_var_mean", 
                                      "map_mean", "map_var_mean"))],
      lower.panel = NULL, upper.panel = upper.panel)
