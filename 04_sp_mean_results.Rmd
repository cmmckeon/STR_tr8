...
for(i in r){
  ...
  s <- droplevels(sum[[i]][which(!is.na(main))])
  s$main <- factor(s$main,levels = c( "MAP_var","MAT_var","MAP", "MAT","Vel", "HF"))
  int <- droplevels(sum[[i]][which(is.na(main))])
  
  p[[i]] <-  ggplot(s,  aes(rn, post.mean, colour = main)) + 
      geom_hline(yintercept= 0, alpha = 0.8) +
  geom_line() +
  geom_pointrange(data = s,
                    aes(rn, post.mean, colour = main, ymin= `l-95% CI`, ymax= `u-95% CI`), 
                    position = position_dodge(0.5), size = 1.5) +
    scale_color_viridis(discrete = TRUE) +
    ...
    geom_pointrange(data = int, colour = "grey",
                    aes(rn, post.mean, colour = "grey" , ymin= `l-95% CI`, ymax= `u-95% CI`),
                    position = position_dodge(0.5), size = 1.5) +
