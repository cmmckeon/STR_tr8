...
for(i in r){
  ...
  s <- droplevels(sum[[i]][main == "main"])
  int <- droplevels(sum[[i]][main == "interaction"])
  
  p[[i]] <-  ggplot(s,  aes(rn, post.mean, colour = rn)) + 
      geom_hline(yintercept= 0, alpha = 0.8) +
  geom_line() +
  geom_pointrange(data = s,
    ...
    geom_pointrange(data = int,
                    aes(rn, post.mean, colour = "interaction", ymin= `l-95% CI`, ymax= `u-95% CI`),
                    position = position_dodge(0.5), size = 1.5) +
