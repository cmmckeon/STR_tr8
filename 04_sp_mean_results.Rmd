...
for(i in r){
  ...
  l[,3][l$r == i] <- round(m_metric_hf[[i]][["hf"]][[1]][["DIC"]],2)
  l[,4][l$r == i] <- round(m_metric_clim_hf[[i]][["hf"]][[1]][["DIC"]],2)
  l[,5][l$r == i] <- round(m_metric_vel[[i]][["hf"]][[1]][["DIC"]],2)
  l[,6][l$r == i] <- round(m_metric_clim_vel[[i]][["hf"]][[1]][["DIC"]],2)
  l[,7][l$r == i] <- round(m_metric_null[[i]][["hf"]][[1]][["DIC"]],2)
}
names(l) <- c("Metrics", "Climate", "Human_footprint", "Climate*Human_footprint", "Velocity","Climate*Velocity", "null")
l[,2] <- as.numeric(l[,2])
l[,3] <- as.numeric(l[,3])
l[,4] <- as.numeric(l[,4])
l[,5] <- as.numeric(l[,5])
...
sum <- list()
for(i in r) {
sum[[i]] <- as.data.frame(summary(m_metric_clim_vel[[i]][["hf"]][[1]])$solutions);
sum[[i]] <- setDT(sum[[i]], keep.rownames = TRUE)[]
sum[[i]] <- sum[[i]][2:6,]
sum[[i]]$rn <- c( "Vel", "MAP", "MAT", "MAP_var","MAT_var")
sum[[i]]$rn <- factor(sum[[i]]$rn,
                      levels = c( "MAP_var","MAT_var","MAP", "MAT",  "Vel"))
}
