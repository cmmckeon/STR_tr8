...
for(i in names(mydata[which(names(mydata) %in% c("effective.mesh.size", "mean.shape.index", "prop.landscape", "total.area", "perimeter.area.frac.dim", "range.size"))])) {
   plot(log(mydata[,i]) ~ log(height), data = mydata, col = "dark grey", bty = "n")
  abline(c[,1][c$hh== "mean" & c$rr ==i], c[,2][c$hh== "mean" & c$rr ==i], col = "red", lwd = 4)
  abline(c[,1][c$hh== "median" & c$rr ==i], c[,2][c$hh== "median" & c$rr ==i], col = "blue", lwd = 4)
  abline(c[,1][c$hh== "max" & c$rr ==i], c[,2][c$hh== "max" & c$rr ==i], col = "green", lwd = 4)
  abline(c[,1][c$hh== "coeff_var" & c$rr ==i], c[,2][c$hh== "coeff_var" & c$rr ==i], col = "purple", lwd = 4)
  }
