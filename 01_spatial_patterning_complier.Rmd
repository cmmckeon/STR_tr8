...
```{r}
metrics <- read.csv("tr8_range_metrics.csv") ## for now this is the updated metrics as calculated by Anna Csergo in spring 2019
```
...
```{r}
mydata <- unique(merge(metrics, traits, by.x = "Species", by.y = "Species.x", all.x = TRUE)) ## gonna leave in all the vars for now...
MDF <- mydata
```
