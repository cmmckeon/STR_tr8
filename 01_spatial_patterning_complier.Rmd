...
metrics <- unique(metrics[, which(names(metrics) %in% c("Species", "effective.mesh.size", "mean.shape.index", "prop.landscape", "total.area", "perimeter.area.frac.dim", "n.cells"))])
mydata <- unique(merge(metrics, height, by.x = "Species", by.y = "species", all.x = T))
mydata$n.cells <- as.numeric(mydata$n.cells)
```


```{r}
m_height <- list()
#range_metrics <- c("effective.mesh.size", "mean.shape.index", "prop.landscape", "total.area", "perimeter.area.frac.dim", "n.cells")
for(i in names(mydata[which(names(mydata) %in% c("effective.mesh.size", "mean.shape.index", "prop.landscape", "total.area", "perimeter.area.frac.dim", "n.cells"))])){
  par(mfrow = c(2,2))
...
```{r}
## get the median values for each species

for(i in levels(mydata$Species)){
  mydata$meid[mydata$Species == i] <- mean(mydata$height[mydata$Species == i])
}

mydata <- unique(mydata[, which(names(mydata) != "height")])
```
