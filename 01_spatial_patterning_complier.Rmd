...
for(i in names(mydata[which(names(mydata) %in% c("effective.mesh.size", "mean.shape.index", "prop.landscape", "total.area", "perimeter.area.frac.dim", "range.size"))])){
  plot(log(mydata[,i]) ~ log(height), data = mydata, main = paste(i), ylab = paste("log", i, sep = " "))
  abline(m_indiv[["mean"]][[i]], col = "red", lwd = 2)
  abline(m_indiv[["median"]][[i]], col = "blue", lwd = 2)
  abline(m_indiv[["max"]][[i]], col = "green", lwd = 2)
  abline(m_indiv[["coeff_var"]][[i]], col = "purple", lwd = 2)
  }
