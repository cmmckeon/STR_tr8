#4thyearscript
#reading in file on brain_bodymass
#comments are above the line of code they refere to
...
#c("aa", "bb", "cc", "dd", "ee") 
#[1] "aa" "bb" "cc" "dd" "ee"
#Incidentally, the number of members(data entries in a column) in a vector is given by the length function.
#length(c("aa", "bb", "cc", "dd", "ee")) 
#[1] 5

#opens a help window explaining all about the command and it's variations
#?command 
#ie. ?barpolt or ?hist
...
#shows you the first few lines of dataset
head(mydata)
#shows internal strucutre of data
str(mydata)
#gives subset of data to work with, including only columns 1 to 6
mydata <- mydata[, 1:6]
#finds blanks for you (NA's) gives you "FALSE" when NOT missing values (NA), so missing values come up as TRUE
idx <- is.na(mydata)
#gives quick look at data now that entries are "FALSE"(not NA) or "TRUE" (NA)
head(idx)
#to see whole datset as indexed (entries described as "TRUE" (NA) or "FALSE" (not NA))
#print(idx)
#to select only subset of data, made of points with NO missing values
aa <- complete.cases(mydata)
# create a subset of the dataframe with only complete casese
complete.data <- mydata[aa, ]
print(complete.data)

#*************
# $ tell R which column (heading) you want to interact with data points from
#() tells R which subset you want to work with
#*************

# extract the indices of the body masses sorted by order of increasing size
idx <- order(complete.data$body_mass_g)

# now print the contents of the test.data dataframe rearranged in this order
#print(mydata[idx, ])
head(mydata[idx, ])
#gives data indexed in order of increasing brain mass
idx <- order(complete.data$brain_mass_g)
head(mydata[idx, ])

# Make sure only Mammals and Birds are present:
mydata <- mydata[which(mydata$class != 'Reptilia'),]
# find the unique entries in this vector
print(unique(mydata$class))
#changes all entries from "class" to lower case
mydata$class <- tolower(mydata$class)

# assuming we have now managed to spell the first few letters of mammalia correctly on each entry we can 
# do a find and replace job using grepl() which performs string matching and returns a logical vector
# of TRUE and FALSE entries.
idx.mam <- grepl("mam", mydata$class)

# and now we replace the TRUE ones with "mammalia"
mydata$class[idx.mam] <- "mammalia"

# searching for duplicates is also possible, and in our example, is more suited to the species names
dupes <- duplicated(mydata$common_name)

#tells R to read ratio column as data points from brainmass divided by corresponding point from bodymass column
#not sure why we tell R this here though. Why do we need to say this?
mydata$ratio <- mydata$brain_mass_g / mydata$body_mass_g

## Plotting the dataframe
hist(mydata$brain_mass_g)
boxplot(mydata$body_mass_g ~ mydata$class)
boxplot(mydata$brain_mass_g ~ mydata$class)

#tells R to read "lbody" as log to the base 10 of bodymass data points
lbody<-log10(mydata$body_mass_g)
lbrain<-log10(mydata$brain_mass_g)
boxplot(lbody ~ mydata$class)
boxplot(lbrain ~ mydata$class,col=c("darkorchid","steelblue4"))

#If we want to keep this *clean* dataset, we should write it to memory.
write.csv(mydata, "brain_data_clean.csv")

#*******************
#Practice coding
#Generating normal datasets
#*******************

Coinflips <-rnorm(n=1000, m=5, sd=2.6)
hist(Coinflips)
#Error in plot.new() : figure margins too large
#happened because bottom right hand window (for files, plots, packages, export ect)
#was too small. Internet suggested - par(mar) to change graphic
#parameters(margins) but didn't run for me. Now its fine
hist(Coinflips)
...
splistnodash <-gsub("_", " ", splist)
minedtraits <- tr8(splistnodash, download_list = NULL, gui_config = TRUE, synonyms=TRUE)
...
#*********************************
#my original plan to create a species list
#not needed, can simple set unique species as a vector

#extracting data from databases on one species only, using tr8 function
tr8("Abies_alba", download_list = NULL, gui_config = TRUE, synonyms=TRUE)
...
#there are some repeated elements (species names), so need to exclude duplicates
# cat(paste(shQuote(unique(Species.x, type="cmd")), collapse=", "))

#not needed, can simple set unique species as a vector
#however, still unsure why this method produced +, even though all brackets appeared closed.
