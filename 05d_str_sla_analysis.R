...
plot((total.area) ~ (sla_max), data = comp_data$data)
plot((range.size) ~ (sla_max), data = comp_data$data)
plot((effective.mesh.size) ~ (sla_max), data = comp_data$data)
plot(mean.shape.index~ (sla_max), data = comp_data$data)
plot(prop.landscape~ (sla_max), data = comp_data$data)
plot((perimeter.area.frac.dim) ~ (sla_max), data = comp_data$data)
...
#   MCMCglmm(fixed = (perimeter.area.frac.dim) ~ (sla_max),
#           #random = ~ animal,
#            rcov = ~units,
#            family= "gaussian",
...
for(i in names(sla[which(names(sla) %in% c("total.area", "range.size", "effective.mesh.size", "mean.shape.index", "prop.landscape", 
  ...
  plot(f[[i]], data = comp_data$data, col = "grey", cex = 0.7, cex.lab = 1.5, cex.main = 1.8,
       ylab = paste(k[[i]]), main = paste(k[[i]]), xlab = paste(" max sla"), bty = "n")
