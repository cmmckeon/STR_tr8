#' ---
#' title: "05f_str_woodiness_analysis"
...
# ## main effects
# woodiness$woodiness.wec <- factor(woodiness$woodiness)
# contrasts(woodiness$woodiness.wec) <- contr.wec(woodiness$woodiness, "therophyte")
...
# for (i in names(Filter(is.factor, woodiness))) {
#   plot(woodiness[,i],
#        main = paste(i),
#        xlab = paste(i))
# }
...
for(j in names(comp_data[["data"]][which(names(comp_data[["data"]]) %in% c("total.area", "range.size", "effective.mesh.size", "mean.shape.index",
  ...
  m_indiv_woodiness[[j]][["woodiness"]] <-mod_list <- mclapply(1:2, function(i) {
  ...
  mod_mcmc <-  m_indiv_woodiness[["woodiness"]][[j]][[1]]
  mod_mcmc_2 <-  m_indiv_woodiness[["woodiness"]][[j]][[2]]}

#saveRDS(m_indiv_woodiness, "m_woodiness_phlyo_parexp.rds")
...
mod_mcmc <- m_indiv_woodiness[[z]][["woodiness"]][[1]]
mod_mcmc_2 <- m_indiv_woodiness[[z]][["woodiness"]][[2]]
...
for(i in names(woodiness[which(names(woodiness) %in% c("total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
  ...
  plot(f[[i]], data = comp_data$data, col = "grey", cex = 0.7, cex.lab = 1.5, cex.main = 1.8,
       ylab = paste(k[[i]]), main = paste(k[[i]]), xlab = paste("woodiness"), bty = "n")
