...
for(j in cont){
  ...
  a$mean[a$species == i] <- mean(a[,2][a$species == i])
...
for(i in cont){
    ...
    hist(log(x[[i]][,j]), main = paste(i,j), xlab = paste("log",j), cex = 0.2, breaks = 50)
}}
```
...
# # }
# 
# b <- unique(merge(woodiness, lifeform, by = "species"))
# b <- unique(merge(b, height, by = "species"))
# b <- unique(merge(b, seed_mass, by = "species"))
# b <- unique(merge(b, sla, by = "species"))
# b <- unique(merge(b, leaf_area, by = "species"))
# b <- drop_na(b)
```


## clean but not tidy as far as here.
