...
```{r}
## Extract each dataset
height <- unique(drop_na(mydata[, which(names(mydata) %in% c("species", "total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim", "height_max"))]))
leaf_area <- unique(drop_na(mydata[, which(names(mydata) %in% c("species", "total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim","leaf_area_max"))]))
lifeform <- unique(drop_na(mydata[, which(names(mydata) %in% c("species","total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim","lifeform"))]))
seed_mass <- unique(drop_na(mydata[, which(names(mydata) %in% c("species", "total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim","seed_mass_max"))]))
sla <- unique(drop_na(mydata[, which(names(mydata) %in% c("species", "total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim","sla_max"))]))
woodiness <- unique(drop_na(mydata[, which(names(mydata) %in% c("species", "total.area", "range.size", "effective.mesh.size", "mean.shape.index", 
                                                      "prop.landscape", "perimeter.area.frac.dim","woodiness"))]))
```


```{r}

for (i in names(Filter(is.numeric, woodiness[, which(names(woodiness) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  hist(log(height[,i]), main = paste(i)) 
}

for (i in names(Filter(is.numeric, woodiness[, which(names(woodiness) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  hist((height[,i]), main = paste(i)) 
}
...
for (i in names(Filter(is.numeric, height[, which(names(height) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  height[, i] <- c(log(height[,i])) 
}
for (i in names(Filter(is.numeric, leaf_area[, which(names(leaf_area) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  leaf_area[, i] <- c(log(leaf_area[,i])) 
}
for (i in names(Filter(is.numeric, seed_mass[, which(names(seed_mass) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  seed_mass[, i] <- c(log(seed_mass[,i])) 
}
for (i in names(Filter(is.numeric, sla[, which(names(sla) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  sla[, i] <- c(log(sla[,i])) +10
}
for (i in names(Filter(is.numeric, lifeform[, which(names(lifeform) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  lifeform[, i] <- c(log(lifeform[,i])) 
}
for (i in names(Filter(is.numeric, woodiness[, which(names(woodiness) %nin% c("mean.shape.index", "prop.landscape"))]))) {
  woodiness[, i] <- c(log(woodiness[,i])) 
}
...
## Numeric variables
for (i in names(Filter(is.numeric, g))) {
  hist(log(g[,i]),
       breaks = 3000,
       main = paste(i),
       xlab = paste(i))
