...
metrics$Species <- gsub(" ssp.*", "", metrics$Species) ## 701 unique species
...
metrics <- metrics[metrics$species != "Helleborus lividus",] ## had infinity value
...
length(unique(metrics$species)) ## 700 unique species
```
...
STILL TO DO - clean life form species names!!!!
```{r}
b <- data.frame(unique(mydata$species))
names(b) <- "species"
for (i in cont){
