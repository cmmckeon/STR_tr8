...
par(mfrow=c(1,1))
plot(log(aggregation.index) ~ log(canopy_height), bty = "l", data = OA)
plot(log(aggregation.index) ~ log(ncells), bty = "l", data = OA)
m1 <-glm(log(aggregation.index) ~ log(canopy_height)+log(ncells), 
...
plot(log(aggregation.index) ~ log(seed_number_per_shoot), data = OA)
...
par(mfrow = c(1,1),bty="l")
plot(log(aggregation.index) ~ log(seed_mass), data = OA)
...
NZ <- OA[which(aggregation.index > 0),]
CHECK <- OA[which(OA$aggregation.index < 1),]
